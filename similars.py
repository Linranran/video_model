...
def difference(hist1,hist2):
    ...
    return sum
#     similar = sum(
#         1 - (0 if l == r else float(abs(int(l) - int(r))) / max(l, r)) for l, r in zip(hist1, hist2)) / len(
#         hist1)
#     return similar
...
def similar(img_hist,type='similar'):
    ...
    if type=='similar':
        for i in range(len(img_hist)-1):
            similar=sum(1 - (0 if l == r else float(abs(int(l) -int(r)))/max(l, r)) for l, r in zip(img_hist[i], img_hist[i+1]))/len(img_hist[i])
    ...
    if type ==  'ssim':
        """the input of ssim loss must be matrix
        """
        similars = []
        for i in range(len(img_hist) - 1):
            similars.append(compare_ssim(img_hist[i], img_hist[i + 1]))
        return similars

    if type ==  'sigmoid_similar':
        similars = []
        for i in range(len(img_hist) - 1):
            sim = sum(1 - (0 if l == r else float(abs(int(l) - int(r))) / max(l, r)) for l, r in
                      zip(img_hist[i], img_hist[i + 1])) / len(img_hist[i])
            similars.append(sim)
        return similars

    if type ==  'Cosine_similar':
        similars = []
        for i in range(len(img_hist) - 1):
            sim =scipy.spatial.distance.pdist(np.vstack((img_hist[i], img_hist[i + 1])), 'cosine')
            similars.append(sim)
        return similars

    if type ==  'euclidean_similar':
        similars = []
        for i in range(len(img_hist) - 1):
            sim=scipy.spatial.distance.pdist(np.vstack((img_hist[i], img_hist[i + 1])), 'euclidean')
