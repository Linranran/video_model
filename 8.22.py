...
import cv2
import numpy as np
from tools import draw,get_img,feng_difference,pattern,sample_hist,pixel_hist,sample_refresh_hist,get_img_filter,ssim,detect_pattern,scene_change,re_detect_pattern
...
def draw_one_video(name):
    ...
    while (ret):
        ...
        # important_frame=frame_mum - args.frame_num + scene
        ...
        #         print('lock_patterns', lock_patterns)
            ...
            # patterns = pattern(similars)
                lock_patterns = detect_pattern(similars, args.FR)
                print('locked the video pattern',lock_patterns)
                draw(similars, name)
...
if __name__ == '__main__':
    ...
    #     draw_one_    video('../data/962a/coco_3.mp4')
